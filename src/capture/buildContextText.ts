...
export function buildContextText(d: ElementData): string {
	...
	L.push('CSS:');
	L.push('```css');
	for (const r of d.cssMatched ?? []) L.push(r);
	if (d.cssInherited?.length) {
		L.push('');
		L.push('/* Inherited */');
		for (const r of d.cssInherited) L.push(r);
	}
	if (d.cssResolved?.length) {
		L.push('');
		L.push('/* Resolved values */');
		for (const r of d.cssResolved) L.push(r);
	}
	if (d.cssVars?.length) {
		L.push('');
		L.push('/* CSS variables */');
		for (const v of d.cssVars) L.push(v);
	}
	L.push('```');
	return L.join('\n');
}
