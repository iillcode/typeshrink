...
export function activate(context: vscode.ExtensionContext) {
	...
	function commitStyleEdits(ecbId: string | null, edits: Array<{ prop: string; value: string }>) {
		...
		const mergedOrder: string[] = order.slice();
		const mergedBy = new Map(byProp);
		for (const prev of path.edits || []) {
			if (!mergedBy.has(prev.prop)) { mergedOrder.push(prev.prop); }
			else if (!mergedOrder.includes(prev.prop)) { mergedOrder.unshift(prev.prop); }
			// note: new values already in byProp win; untouched old props persist below
			if (!mergedBy.has(prev.prop)) mergedBy.set(prev.prop, prev.value);
		}
