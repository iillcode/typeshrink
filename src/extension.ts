...
function mergeCookieHeaders(existing: string | undefined, jar: CookieJar | undefined, url: URL): string | undefined {
...
class InjectingProxy {
	...
	start(targetOrigin: string): Promise<number> {
...
function getWebviewHtml(proxyPort: number): string {
	return `<!DOCTYPE html>
<html>
...
export function activate(context: vscode.ExtensionContext) {
	...
	const storedJars = new Map<string, JarCookie[]>();
	const liveJars = new Map<string, CookieJar>();
	try {
		if (fs.existsSync(jarsFile)) {
			const raw = JSON.parse(fs.readFileSync(jarsFile, 'utf8')) as Record<string, JarCookie[]>;
			for (const [origin, list] of Object.entries(raw)) storedJars.set(origin, list);
		}
	} catch (e: any) {
	...
	function dropJar(origin: string): void {
	...
	function persistJars(): void {
		try {
			...
			for (const [origin, jar] of liveJars) obj[origin] = jar.toJSON();
			for (const [origin, list] of storedJars) if (!obj[origin]) obj[origin] = list;
		...
		resolveWebviewView(view: vscode.WebviewView) {
			...
			view.webview.onDidReceiveMessage((msg) => {
				if (msg.type === 'copy') {
					...
					showDetails(msg.data as ElementData, true);
	...
	class ElementsTreeProvider implements vscode.TreeDataProvider<ElementItem> {
	...
	context.subscriptions.push(
					...
					(msg) => {
						if (msg.type === 'elementClicked') {
							...
							vscode.window.showInformationMessage(`Page: ${u} | ${history.length} element(s) captured`, 'Clear history')
								.then(pick => { if (pick === 'Clear history') vscode.commands.executeCommand('elementClickBrowser.clearHistory'); });
		...
		vscode.commands.registerCommand('elementClickBrowser.clearSession', async () => {
			...
			if (originInput.trim() === '') {
				for (const origin of [...liveJars.keys(), ...storedJars.keys()]) dropJar(origin);
				liveJars.clear();
				storedJars.clear();
				persistJars();
	...
	context.subscriptions.push({ dispose: () => { /* proxy disposed via panel.onDidDispose */ } });
}
