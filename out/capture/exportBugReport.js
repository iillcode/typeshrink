...
function styleEditsReport(p) {
    ...
    if (el.id)
        L.push('- id: `' + el.id + '`');
    if (el.className)
        L.push('- class: `.' + String(el.className).trim().split(/\s+/).join('`.`') + '`');
    if (el.rect)
        L.push('- Size/position at capture: ' + Math.round(el.rect.w) + '×' + Math.round(el.rect.h) + ' @ (' + Math.round(el.rect.x) + ', ' + Math.round(el.rect.y) + ')');
    if (el.source && el.source.file)
        L.push('- Source: `' + el.source.file + ':' + el.source.line + '`');
